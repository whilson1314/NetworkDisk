...
PDU *handleFlushFriendRequest(PDU *pdu)
{
    ...
    {
        strncpy((char*)(resPdu->caMsg) + 36 * i, strList.at(2 * i).toStdString().c_str(), 32);
...
PDU *handleDeleteFriendRequest(PDU *pdu)
{
    ...
    strncpy(sourceName, pdu->caData + 32, 32);
    bool ret = DBoperate::getInstance().handleDeleteFriend(deleteName, sourceName);
...
PDU *handlePrivateChatRequest(PDU *pdu)
{
    ...
    strncpy(sourceName, pdu->caData + 32, 32);
    qDebug() << "handlePrivateChatReuqest " << chatedName << " " << sourceName;
...
// 删除文件或文件夹处理
PDU *handleDeleteFileOrDirRequest(PDU *pdu)
{
    PDU* resPdu = mkPDU(0);
...
void MyTcpSocket::recvMsg()
{
    ...
    case ENUM_MSG_TYPE_SEARCH_USERS_REQUEST:    //查找用户
    {
    ...
    {
        resPdu = handleDeleteFileOrDirRequest(pdu);
        break;
    }
