    ...
    : QWidget{parent}
{
    ...
    m_pMoveDesDirPB = new QPushButton("目标目录");
    m_pMoveDesDirPB -> setEnabled(false);           //设置目标文件不可点击
    ...
    pFileOpVBL -> addWidget(m_pMoveDesDirPB);
    ...
    connect(m_pMoveFilePB, SIGNAL(clicked(bool)),
            this, SLOT(moveFile()));
    connect(m_pMoveDesDirPB, SIGNAL(clicked(bool)),
...
void FileSystem::delFileOrDir()
{
    ...
    QString strDelPath = QString("%1/%2").arg(strCurPath).arg(strFileName); // 要删除的文件路径
...
{
    QString strCurPath = TcpClient::getInstance().getStrCurPath();
    QListWidgetItem *qItem = m_pFileListW -> currentItem();         // 获取当前选中文件
    if (NULL == qItem)
    {
        QMessageBox::warning(this, "重命名文件", "请选择需要重命名的文件！");
        return ;
    }
    QString strOldName = qItem -> text().split('\t')[0];        // 获取旧的文件名
...
void FileSystem::moveFile()
{
    ...
    m_pMoveDesDirPB->setEnabled(true); // 设置目标目录可点击
...
void FileSystem::moveDesDir()
{
    ...
    if(sbMoveAffirm == QMessageBox::No) // 不移动
    {
        ...
        m_pMoveDesDirPB->setEnabled(false);
        return ;
    }
...
void FileSystem::shareFile()
{
    ...
    // 获得好友列表
    QListWidget *friendLW = OpeWidget::getInstance().getFriend()->getPFriendLW();
    // 选择好友窗口展示

    m_pSharedFileLW->updateFriendList(friendLW);

    if(m_pSharedFileLW->isHidden()) // 如果窗口隐藏，则显示出来
    {
        m_pSharedFileLW->show();
    }
