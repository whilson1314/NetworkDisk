#include "sharedfilefeiendlist.h"
#include <QDebug>
#include "opewidget.h"
#include "tcpclient.h"

sharedFileFeiendList::sharedFileFeiendList(QWidget *parent)
    : QWidget{parent}
{
...
void sharedFileFeiendList::updateFriendList(QListWidget *pFriendList)
{
    if(NULL == pFriendList)
    {
        return ;
    ...
    {
        tmp = preFriendList[i];
        m_pFriendsVBL->removeWidget(preFriendList[i]);
        m_pFriendsBG->removeButton(preFriendList[i]);
...
void sharedFileFeiendList::selectAll()
{
...
void sharedFileFeiendList::cancleSelect()
{
    QList<QAbstractButton*> friendsButtons = m_pFriendsBG->buttons();
...
void sharedFileFeiendList::affirmShare()
{
    QString strFileName = OpeWidget::getInstance().getPFileSystem()->getStrSharedFileName();
    QString strFilePath = OpeWidget::getInstance().getPFileSystem()->getStrSharedFilePath();
...
void sharedFileFeiendList::cancleShare()
{
    this -> hide();
}
